...
def valid(w, b, X_valid, Y_valid,w2):
    valid_data_size = len(X_valid)
    x_v2 = np.square(X_valid)

    z = (np.dot(X_valid, np.transpose(w)) + b + np.dot(x_v2, np.transpose(w2)) )
    y = sigmoid(z)
    y_ = np.around(y)
...
def train(X_all, Y_all, save_dir):
    ...
    for epoch in range(1, epoch_num):
        ...
        if (epoch) % save_param_iter == 0:
            ...
            np.savetxt(os.path.join(save_dir, 'w2'), w2)
            np.savetxt(os.path.join(save_dir, 'b'), [b,])
            print('epoch avg loss = %f' % (total_loss / (float(save_param_iter) * train_data_size)))
            total_loss = 0.0
            valid(w, b, X_valid, Y_valid,w2)
        ...
        for idx in range(step_num):
            ...
            z = np.dot(X, np.transpose(w)) + b + np.dot(x_t2, np.transpose(w2))
            y = sigmoid(z)
            
            cross_entropy = -1 * (np.dot(np.squeeze(Y), np.log(y)) + np.dot((1 - np.squeeze(Y)), np.log(1 - y)))#+ rg*np.square(np.transpose(w))+rg*np.square(np.transpose(w2))
            total_loss += cross_entropy

            w_grad = np.sum(-1 * X * (np.squeeze(Y) - y).reshape((batch_size,1)), axis=0)- rg * w
            dw_t = np.sqrt(np.square(dw_t) +  np.square(w_grad))
            
            b_grad = np.sum(-1 * (np.squeeze(Y) - y))#- rg * b
            db_t = np.sqrt(np.square(db_t) + np.square(b_grad))
            
            dw2 = np.sum(-1 * x_t2 * (np.squeeze(Y) - y).reshape((batch_size,1)), axis=0)- rg * w2
            dw2_t = np.sqrt(np.square(dw2_t) +  np.square(dw2))
            
            # adagrad updating parameters
            m1 = rd * m1 - l_rate * (w_grad/dw_t)
            w = w + m1
            
            m0 = rd * m0  - l_rate * b_grad/db_t
            b = b + m0
            
            m2 = rd * m2 - l_rate * (dw2/dw2_t)    
            w2 = w2 + m2
...
def infer(X_test, save_dir, output_dir):
    ...
    w2 = np.loadtxt(os.path.join(save_dir, 'w2'))
    # predict
    z = (np.dot(X_test, np.transpose(w)) + b + np.dot(np.square(X_test), np.transpose(w2)))
    y = sigmoid(z)
    y_ = np.around(y)
