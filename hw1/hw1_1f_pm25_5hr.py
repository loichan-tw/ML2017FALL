...
#          pm10_list.append(float(v))
  for i in range(5760):
    if pm25_list[i] == -1:
      ...
      tmp = pm25_list[i]
  x  = np.zeros((5652,5),dtype = np.float)
  y_ = np.zeros((5652,1),dtype = np.float)
  for m in range(12):
    for d in range(471):
      x[471*m+d,:5] = pm25_list[m*480+d+4:m*480+d+9]
...
#  y_ = (y_-y_mean)
  ...
  w  = 1e-13*np.random.random((5,1))
  dw = np.zeros((5,1),dtype = np.float)
  dw_t = np.zeros((5,1),dtype = np.float)
  m1 = np.zeros((5,1),dtype = np.float)
  '''
  ...
  for i in range(10000):
    ...
    dw = np.sum(-x_t*(e),axis = 0).reshape(5,1) - rg * w
...
def test(w,b):
  x_te = np.zeros((240,5),dtype = np.float)
...
#        x_te[id_,18:] = vector[2:]
  ...
  with open('./5/5hr_pm25test.csv','w') as fp:
    fp.write('id,value\n')
    for i in range(240):
      fp.write('id_' + str(i) + ',' + str(y_te[i,0]) + '\n')

[x,y_] = gendata()
[w,b] = linearRegression(x,y_)
test(w,b)
