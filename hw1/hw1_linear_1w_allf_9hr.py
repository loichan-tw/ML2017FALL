...
def test(w,b):
  ...
  with open('./9/9hr_all_feature.csv','w') as fp:
    fp.write('id,value\n')
    for i in range(240):
...
ax.set_title(r'RMSE 9hr - all feature')
ax.set_xlabel('number of iteration')
ax.set_ylabel('root mean square error')
#ax.plot([0,1,2], [10,20,3])
fig.savefig('./9/9hr_rms_2.png')   # save the figure to file
plt.show()

f = open("./9/w_hw.dat", "wb")
pickle.dump(w,f)
f.close()
f = open("./9/rmstrain.dat", "wb")
pickle.dump(rms,f)
f.close()
f = open("./9/rmstest.dat", "wb")
pickle.dump(rms2,f)
f.close()
f2 = open("./9/b_hw.dat", "wb")
pickle.dump(b,f2)
f2.close()
test(w,b)
